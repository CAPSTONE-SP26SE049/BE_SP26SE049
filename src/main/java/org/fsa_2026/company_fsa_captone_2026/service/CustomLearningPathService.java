...
public class CustomLearningPathService {
    ...
    public CustomPathResponse createCustomPath(String educatorEmail, UUID studentId, CreateCustomPathRequest request) {
        ...
        for (int i = 0; i < request.getLevelIds().size(); i++) {
            ...
            LearningUnit level = learningUnitRepository.findById(levelId)
                    .orElseThrow(() -> new ApiException(NOT_FOUND, "Không tìm thấy Chương với ID: " + levelId));
    ...
    public void submitProgress(UUID studentId, UUID quizId, Integer score) {
        CustomLearningPath path = pathRepository.findFirstByStudentIdAndIsActiveTrueOrderByCreatedAtDesc(studentId)
                .orElseThrow(() -> new ApiException(NOT_FOUND, "Không tìm thấy lộ trình học của bạn"));

        CustomPathProgress progress = progressRepository.findByCustomPathIdAndLearningUnitId(path.getId(), quizId)
        ...
        List<PathLevelResponse> levelResponses = path.getLevels().stream()
                .map(pl -> {
                    ...
                    return PathLevelResponse.builder()
                            ...
                            .region(level.getParent() != null ? level.getParent().getName() : "Unknown")
